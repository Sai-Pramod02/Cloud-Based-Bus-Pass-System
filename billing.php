...
if(isset($_POST['name'])){
    ...
    $query = "INSERT INTO pass (name, email, contact, date, dest, password) VALUES ('$name', '$email', '$contact', '$date', '$dest', '$password')";
    if ($conn->query($query) === TRUE) {
        $lid = $conn->insert_id;
        $nod = round((strtotime($date) - time())/(60*60*24))+1;
        $result = $conn->query("SELECT price FROM destination WHERE name = '$dest'");
        if ($result->num_rows > 0) {
            ...
            echo "No price found for the selected destination.";
        }
    } else {
