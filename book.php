...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["idCard"]["tmp_name"], $target_file)) {
            echo "The file " . htmlspecialchars(basename($_FILES["idCard"]["name"])) . " has been uploaded.";
        } else {
          ...
          <div>
            <form action="billing.php" method="post" class="p-5 bg-white">
             <div class="row form-group">
              ...
              <div class="col-md-12">
                                <label class="text-black" for="college_email">Email</label>
                                <input required type="email" name="email" id="email" class="form-control">
                                <small class="text-muted">Please enter your  email address.</small>
                            </div>
              </div>
...
<div class="row form-group">
    <div class="col-md-12">
        <label class="text-black" for="idCard">Upload Your ID Card</label>
        <input required type="file" name="idCard" id="idCard" class="form-control">
        <small class="text-muted">Please upload a photo of your ID card.</small>
    </div>
</div>
                </div>
              </div>
